use crate::{CatError, fixed::Fixed};
...
impl Point {
    ...
    fn lerp(self, other: Self, unit: u16) -> Result<Self, CatError> {
        Ok(Self::new(
...
impl Path {
    ...
    pub(crate) fn builder(start: Point) -> Result<PathBuilder, CatError> {
        let mut path = Self::EMPTY;
        path.push(start)?;
        Ok(PathBuilder { path })
    }

    pub(crate) fn points(&self) -> Result<&[Point], CatError> {
        self.points.get(..self.count).ok_or(CatError::InvalidScene)
    }
    ...
    fn push(&mut self, point: Point) -> Result<(), CatError> {
        let slot = self
            .points
            .get_mut(self.count)
            .ok_or(CatError::InvalidScene)?;
        *slot = point;
        self.count = self.count.checked_add(1).ok_or(CatError::NumericRange)?;
        Ok(())
    }
}
...
impl PathBuilder {
    pub(crate) fn line_to(&mut self, end: Point) -> Result<(), CatError> {
        self.path.push(end)
    }

    pub(crate) fn quad_to(&mut self, control: Point, end: Point) -> Result<(), CatError> {
        let start = *self.path.points()?.last().ok_or(CatError::InvalidScene)?;
    ...
    pub(crate) fn finish(mut self, closed: bool) -> Result<Path, CatError> {
...
mod tests {
    ...
    #[test]
    fn curve_lowering_has_a_fixed_bound_and_endpoint() -> Result<(), CatError> {
        let zero = Fixed::ZERO;
