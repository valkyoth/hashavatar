...
identity bytes through fallible constructors and return `CatError`. Production
library paths must not use `panic!`, `assert!`, `unwrap`, `expect`, or
`unreachable!` for caller-reachable validation.
...
`scripts/validate-panic-policy.sh` scans production Rust modules in both
workspace crates. Clippy additionally forbids panic, unwrap, and expect sites.
