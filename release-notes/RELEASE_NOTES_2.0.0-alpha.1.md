...
`2.0.0-alpha.1` is the first Hashavatar 2.0 architecture milestone. Its named
implementation-stop commit is used for pentest and `hashavatar-website`
integration testing; no prerelease tag or crates.io package is created.
Production users remain on `1.3.x`.
